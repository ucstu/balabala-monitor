...
const getIndicatorStatistics = () => {
  getPerformancesBasicindicatorstatistics(requestParam).then(({ data }) => {
...
let indicators = $ref<Array<BasicList & { pageUrl: string }> | undefined>();

const getIndicators = () => {
  getPerformancesBasicindicators(requestParam).then(({ data }) => {
    indicators = data;
  });
};
getIndicators();
...
watchEffect(() => {
  if (indicators?.[activeIndicator]) {
    getPerformancesBasicindicatorstatistics({
      ...requestParam,
      pageUrl: indicators[activeIndicator].pageUrl,
    }).then(({ data }) => {
